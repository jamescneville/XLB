...
from xlb.operator.force.wall_model_momentum_transfer import WallModelMomentumTransfer
from xlb.helper.initializers import CustomInitializer
from xlb.helper.simulation_manager_single_res import SimulationManagerSingleRes
from xlb.utils.single_res_io import SingleResIO, neon_field_to_numpy
...
def prep_inputs(input_file):
    ...
    scm_progress(2)
    print(f"Progress 2%")
    body_vertices, wheel_vertices, wheel_centers, grid_shape_zip, partSize, shift = mesh_prep(
    ...
    # Calculate lattice parameters (single resolution -- no coarsening)
    delta_x_coarse = voxel_size
    ...
    # Define exporter object (single-resolution HDF5+XDMF/slice-image IO --
    # surface-VTK and isosurface-STL export are not implemented, see
    # SingleResIO's docstring).
    field_name_cardinality_dict = {"velocity": 3, "density": 1, "bc_mask": 1}
    h5exporter = SingleResIO(
    ...
    # Create grid (plain single-resolution dense Neon grid)
    grid = grid_factory(
        grid_shape_zip,
    ...
    # Calculate num_steps
    grid_shape_x_coarsest = grid.shape[0]
    ...
    boundary_conditions = setup_boundary_conditions(grid, body_vertices, wheel_vertices, wheel_centers, ulb, lbm_visc, grid_shape_zip, precision_policy, jsonfile, compute_backend)
    scm_progress(10)
    print(f"Progress 10%")
    # Create initializer
    ...
    # Initialize simulation (single resolution -- no mres_perf_opt, no
    # per-level omega scaling)
    sim = SimulationManagerSingleRes(
        omega=omega,
        grid=grid,
    ...
    stats = compute_voxel_statistics_and_reference_area(jsonfile, sim, grid_shape_zip, boundary_conditions, voxel_size, wheel_ids)
    ...
    # Setup momentum transfer (single resolution -- no mres_perf_opt)
    momentum_transfer = WallModelMomentumTransfer(
    ...
    print(f"Total voxels (single resolution, dense): {total_voxels:,}")
    ...
    with open(os.path.join(output_dir, "project.log"),'a') as fd:
        ...
        fd.write(f"Total voxels (single resolution, dense): {total_voxels:,}\n")
...
def mesh_prep(voxel_size, car_mesh, body_mesh, wheel_meshes, output_dir, jsonfile):
    """Single-resolution mesh prep.

    No sparsity pattern / level_data here -- a plain ``dGrid`` is fully
    dense, so there's no "which voxels are active" precomputation needed
    the way multires' sparse mGrid requires. This only needs a bounding-box
    -> grid_shape computation from a single flat ``domain`` fraction block
    (modeled on the multires "cuboid" mesher branch, just without any
    per-level padding/refinement).
    """
    ...
    domain = jsonfile['mesher']['domain']
    shift = np.array(
        [
            domain["-x"] * partSize[0] - min_bound[0],
            domain["-y"] * partSize[1] - min_bound[1],
            domain["-z"] * partSize[2] - min_bound[2],
        ],
        dtype=float,
    )

    # +1.0 per axis accounts for the car's own extent -- domain["-x"]/domain["x"]
    # etc. are margins *beyond* the car (matching how the mres domain fractions
    # worked), not the total span measured from the origin.
    domain_extent = np.array(
        [
            (domain["-x"] + 1.0 + domain["x"]) * partSize[0],
            (domain["-y"] + 1.0 + domain["y"]) * partSize[1],
            (domain["-z"] + 1.0 + domain["z"]) * partSize[2],
        ],
        dtype=float,
    )
    grid_shape_finest = tuple(int(np.ceil(extent / voxel_size)) for extent in domain_extent)
    ...
    print(f"Full shape (single-resolution, uniform {voxel_size} m voxels) is {grid_shape_finest}")
    print(f"Total voxels: {grid_shape_finest[0] * grid_shape_finest[1] * grid_shape_finest[2]:,}")
    ...
    return body_vertices, wheel_vertices, wheel_centers, grid_shape_finest, partSize, shift

# Boundary Conditions Setup
# =========================
def setup_boundary_conditions(grid, body_vertices, wheel_vertices, wheel_centers, ulb, lbm_visc, grid_shape, precision_policy, jsonfile, compute_backend=ComputeBackend.NEON):
    """
    Set up boundary conditions for the simulation (single resolution -- one
    box, no per-level indices/filtering loop).
    """
    box = grid.bounding_box_indices()
    left_indices = box["left"]
    right_indices = box["right"]
    top_indices = box["top"]
    bottom_indices = box["bottom"]
    front_indices = box["front"]
    back_indices = box["back"]

    # Filter front and back indices to remove overlaps with top and bottom
    # (and vice versa) so edge voxels aren't claimed by two BCs at once.
    left_set = set(zip(*left_indices)) if left_indices else set()
    right_set = set(zip(*right_indices)) if right_indices else set()
    top_set = set(zip(*top_indices)) if top_indices else set()
    bottom_set = set(zip(*bottom_indices)) if bottom_indices else set()
    front_set = set(zip(*front_indices)) if front_indices else set()
    back_set = set(zip(*back_indices)) if back_indices else set()
    ...
    filtered_front_indices = [list(coords) for coords in zip(*filtered_front_set)] if filtered_front_set else []
    filtered_back_indices = [list(coords) for coords in zip(*filtered_back_set)] if filtered_back_set else []
    filtered_top_indices = [list(coords) for coords in zip(*filtered_top_set)] if filtered_top_set else []
    filtered_bottom_indices = [list(coords) for coords in zip(*filtered_bottom_set)] if filtered_bottom_set else []
...
def compute_voxel_statistics_and_reference_area(jsonfile, sim, grid_shape, boundary_conditions, voxel_size, wheel_ids=[]):
    """
    Compute totals and reference area for the single-resolution case,
    mirroring the multires approach exactly: read back bc_mask, find solid
    (or car/wheel-BC-id, for RAY voxelization) voxels, and count unique Y-Z
    voxel columns -- the frontal-area projection.

    Single resolution has no per-level active/solid voxel split (there is
    only one level = the whole dense grid), so total_voxels is just the
    domain volume and total_lattice_updates_per_step == total_voxels (no
    acoustic-scaling sub-stepping multiplier).
    """
    sim.macro(sim.f_0, sim.rho, sim.u)

    total_voxels = int(grid_shape[0]) * int(grid_shape[1]) * int(grid_shape[2])
    total_lattice_updates_per_step = total_voxels

    bc_mask_arr = neon_field_to_numpy(sim.bc_mask, 1, grid_shape, wp.uint8)[0]

    if jsonfile["BCtypes"]["car_voxelization"] == "RAY":
        target_ids = [bc.id for bc in wheel_ids]
        target_ids.append(boundary_conditions[0].id)
        is_target_voxel = np.isin(bc_mask_arr, target_ids)
    else:
        is_target_voxel = bc_mask_arr == BC_SOLID

    solid_voxel_indices = np.argwhere(is_target_voxel)
    unique_jk = np.unique(solid_voxel_indices[:, 1:3], axis=0)
...
def solve(
    ...
    # Last measured throughput. Two blocks below can fire on the same step (the 1%
    # progress print and the solution print); whichever runs first consumes the
    # interval, so the second reuses these values instead of dividing by ~0.
    MLUPS = 0.0
    interval_time = 0.0
    time_out = False
    drag_values = []
    ...
    if jsonfile['settings']['debug']:
        for step in range(num_steps):
            ...
            if step % int(num_steps / 100) == 0:
                ...
                if steps_since_last_print > 0:
                    wp.synchronize()
                    measured = time.time() - interval_start
                    if measured > 0:
            ...
            if step % print_output_interval == 0 or step == num_steps - 1 or time_out:
                ...
                # so slice/HDF5 output time is never charged against MLUPS. If the
                # progress block above already consumed this interval, keep its value.
                wp.synchronize()
                measured = time.time() - interval_start
                fresh_mlups = steps_since_last_print > 0 and measured > 0
                if fresh_mlups:
                    interval_time = measured
                    MLUPS = (total_lattice_updates_per_step * steps_since_last_print) / interval_time / 1e6
                sim.macro(sim.f_0, sim.rho, sim.u)
                ...
                if h5exporter is not None:
                    h5exporter.to_slice_image(
                        filename,
                        {"velocity": sim.u},
                        plane_point=(0.5, 0.5, 0.5),  # Y=0.5 -> domain centerline for a Y-normal slice
                        plane_normal=(0, 1, 0),
                ...
                # sim.export_macroscopic writes a plain VTI, independent of h5exporter.
                sim.export_macroscopic(os.path.join(output_dir, f"{jsonfile['outputName']}_u_"))
        ...
        if h5exporter is not None and getattr(h5exporter, "SUPPORTS_MULTI_PLANE_SLICES", True):
            save_slices(output_dir, grid_shape_zip, shift, h5exporter, delta_x_coarse, voxel_size,jsonfile, partSize)
        elif h5exporter is not None:
            print("WARNING: multi-plane outputSlices rendering is not implemented for single-resolution -- skipping.")
            ...
            sim.export_macroscopic(os.path.join(output_dir, f"{jsonfile['outputName']}_average_u_"))
        scm_progress(95)
        print(f"Progress 95%")
        jsonfile['results'] ={}
        ...
        # Always write a plain VTI of the final state, regardless of h5exporter.
        sim.macro(sim.f_0, sim.rho, sim.u)
        sim.export_macroscopic(os.path.join(output_dir, f"{jsonfile['outputName']}_final_u_"))
        ...
        for step in range(num_steps):
            ...
            if (step >= crossover_step and (step % print_interval == 0 or step == num_steps - 1)) or time_out:
                    ...
                    wp.synchronize()
                    measured = time.time() - interval_start
                    if steps_since_last_print > 0 and measured > 0:
                        interval_time = measured
                        MLUPS = (total_lattice_updates_per_step * steps_since_last_print) / interval_time / 1e6
                        print(f"  MLUPS: {MLUPS:.1f}")
                    sim.macro(sim.f_0, sim.rho, sim.u)
